...
user_name = check_markdown(v.user.first_name)
  ...
  save_data(_config.moderation.data, data)
      ...
      save_data(_config.moderation.data, data)
  return '*مجموعة ['..msg.to.title..'] has been added*'
end
...
if lock_tag == "yes" then
 return "*التاك#/@* _Posting Is Already Locked_"
else
...
local lock_tag = data[tostring(target)]["settings"]["lock_tag"]
 if lock_tag == "no" then
return "*التاك#/@* _Posting Is Not Locked_" 
else 
...
save_data(_config.moderation.data, data) 
 return "*Arabic/Persian* تم القفل\n〰〰〰〰〰〰〰〰\n*øяđeявч* : "..msg.from.first_name
end
end
...
return "*Arabic/Persian* تم الفتح\n〰〰〰〰〰〰〰〰\n*øяđeявч*: "..msg.from.first_name
end
end
...
return group_settings(msg, target)
   ...
   if data[tostring(msg.to.id)]['owners'][tostring(msg.reply.id)] then
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is already_ *group owner*"
    else
  data[tostring(msg.to.id)]['owners'][tostring(msg.reply.id)] = username
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is now_ *group owner*"
      end
  ...
  if not getUser(matches[2]).result then
   return "*المستخدم👤 not found*"
    ...
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is already_ *group owner*"
    else
  data[tostring(msg.to.id)]['owners'][tostring(matches[2])] = user_name
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is now_ *group owner*"
   end
  ...
  if not resolve_username(matches[2]).result then
   ...
   if data[tostring(msg.to.id)]['owners'][tostring(status.id)] then
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is already_ *group owner*"
    else
  data[tostring(msg.to.id)]['owners'][tostring(status.id)] = check_markdown(status.username)
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is now_ *group owner*"
   end
end
end
   ...
   if not data[tostring(msg.to.id)]['owners'][tostring(msg.reply.id)] then
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is not_ *group owner*"
    else
  data[tostring(msg.to.id)]['owners'][tostring(msg.reply.id)] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is no longer_ *group owner*"
      end
	  elseif matches[2] and matches[2]:match('^%d+') then
  if not getUser(matches[2]).result then
   return "*المستخدم👤 not found*"
    ...
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is not_ *group owner*"
    else
  data[tostring(msg.to.id)]['owners'][tostring(matches[2])] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is no longer_ *group owner*"
      end
   elseif matches[2] and not matches[2]:match('^%d+') then
  if not resolve_username(matches[2]).result then
   return "*المستخدم👤 not found*"
   ...
   if not data[tostring(msg.to.id)]['owners'][tostring(status.id)] then
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is not_ *group owner*"
    else
  data[tostring(msg.to.id)]['owners'][tostring(status.id)] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is no longer_ *group owner*"
      end
end
end
   ...
   if data[tostring(msg.to.id)]['mods'][tostring(msg.reply.id)] then
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is already_ *group moderator*"
    else
  data[tostring(msg.to.id)]['mods'][tostring(msg.reply.id)] = username
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is now_ *group moderator*"
      end
	  elseif matches[2] and matches[2]:match('^%d+') then
  if not getUser(matches[2]).result then
   return "*المستخدم👤 not found*"
    ...
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is already_ *group moderator*"
    else
  data[tostring(msg.to.id)]['mods'][tostring(matches[2])] = user_name
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is now_ *group moderator*"
   end
   elseif matches[2] and not matches[2]:match('^%d+') then
  if not resolve_username(matches[2]).result then
   return "*المستخدم👤 not found*"
   ...
   if data[tostring(msg.to.id)]['mods'][tostring(user_id)] then
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is already_ *group moderator*"
    else
  data[tostring(msg.to.id)]['mods'][tostring(status.id)] = check_markdown(status.username)
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is now_ *group moderator*"
   end
end
end
   ...
   if not data[tostring(msg.to.id)]['mods'][tostring(msg.reply.id)] then
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is not_ *group moderator*"
    else
  data[tostring(msg.to.id)]['mods'][tostring(msg.reply.id)] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is no longer_ *group moderator*"
      end
	  elseif matches[2] and matches[2]:match('^%d+') then
  if not getUser(matches[2]).result then
   return "*المستخدم👤 not found*"
  ...
  data[tostring(msg.to.id)]['mods'][tostring(matches[2])] = user_name
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..user_name.." `"..matches[2].."` _is no longer_ *group moderator*"
      end
   elseif matches[2] and not matches[2]:match('^%d+') then
  if not resolve_username(matches[2]).result then
   return "*المستخدم👤 not found*"
   ...
   if not data[tostring(msg.to.id)]['mods'][tostring(status.id)] then
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is not_ *group moderator*"
    else
  data[tostring(msg.to.id)]['mods'][tostring(status.id)] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is no longer_ *group moderator*"
      end
end
end
   ...
   if data[tostring(msg.to.id)]['whitelist'][tostring(msg.reply.id)] then
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is already in_ *white list*"
    else
  data[tostring(msg.to.id)]['whitelist'][tostring(msg.reply.id)] = username
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _added to_ *white list*"
      end
  ...
  if not getUser(matches[3]).result then
   return "*المستخدم👤 not found*"
    ...
    return "_المستخدم👤_ "..user_name.." `"..matches[3].."` _is already in_ *white list*"
    else
  data[tostring(msg.to.id)]['whitelist'][tostring(matches[3])] = user_name
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..user_name.." `"..matches[3].."` _added to_ *white list*"
   end
  ...
  if not resolve_username(matches[3]).result then
   ...
   if data[tostring(msg.to.id)]['whitelist'][tostring(status.id)] then
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is already in_ *white list*"
    else
  data[tostring(msg.to.id)]['whitelist'][tostring(status.id)] = check_markdown(status.username)
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _added to_ *white list*"
   end
end
end
   if matches[1] == "البيضاء" and matches[2] == "-" and is_mod(msg) then
   ...
   if not data[tostring(msg.to.id)]['whitelist'][tostring(msg.reply.id)] then
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _is not in_ *white list*"
    else
  data[tostring(msg.to.id)]['whitelist'][tostring(msg.reply.id)] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..username.." `"..msg.reply.id.."` _removed from_ *white list*"
      end
	  elseif matches[3] and matches[3]:match('^%d+') then
  if not getUser(matches[3]).result then
   return "*User not found*"
    ...
    return "_المستخدم👤_ "..user_name.." `"..matches[3].."` _is not in_ *white list*"
    else
  data[tostring(msg.to.id)]['whitelist'][tostring(matches[3])] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ "..user_name.." `"..matches[3].."` _removed from_ *white list*"
      end
   elseif matches[3] and not matches[3]:match('^%d+') then
  if not resolve_username(matches[3]).result then
   return "*المستخدم👤 not found*"
   ...
   if not data[tostring(msg.to.id)]['whitelist'][tostring(status.id)] then
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _is not in_ *white list*"
    else
  data[tostring(msg.to.id)]['whitelist'][tostring(status.id)] = nil
    save_data(_config.moderation.data, data)
    return "_المستخدم👤_ @"..check_markdown(status.username).." `"..status.id.."` _removed_ *white list*"
      end
end
end
